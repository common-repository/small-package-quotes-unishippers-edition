...
class Unishippers_EnUnishippersSmallFdo
{
    ...
    public function en_cart_package($package)
    {
        ...
        $this->en_fdo_meta_data['plugin_name'] = 'unishippers_small';
        $accessorials['residential'] = get_option('unishepper_small_quote_as_residential_delivery') == 'yes' ? true : false;
    ...
    public function en_package_items($items)
    {
        ...
        foreach ($items as $item_key => $item_data) {
            $productId = $productName = $productQty = $actualProductPrice = $products = $productPrice = $productWeight = $productLength = $productWidth = $productHeight = $ptype = $hazardousMaterial = $productType = $productSku = $productClass = $attributes = $variantId = $markup = $product_signature = '';
            $attributes = [];
            extract($item_data);
    ...
    public function get_fdo_coupon_data_from_api(){
        ...
        $url = UNISHIPPERS_FDO_COUPON_BASE_URL . "/use_coupon";
    ...
    public function get_fdo_coupon_parsed_data($fdo_coupon_data)
    {
        ...
        $parsed_data['register_url'] = UNISHIPPERS_FDO_COUPON_BASE_URL.'/register?code='.$base64string;
        $parsed_data['login_url'] = UNISHIPPERS_FDO_COUPON_BASE_URL.'/login?code='.$base64string;
        // $parsed_data['user_guide_get_started'] = UNISHIPPERS_FDO_COUPON_BASE_URL.'/help/Getting_Started';
