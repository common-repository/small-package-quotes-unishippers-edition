...
$va_obj = new \Unishippers_EnUnishippersSmallVa();
    ...
    if(empty($va_coupon_data)){
            ...
            ?>
                <div id="message" class="en-coupon-code-div woocommerce-message en-coupon-notice">
                    <p>
                        ...
                        If you don’t have one, get validate addresses free for one year by using promo code <strong>[<?php echo esc_attr( $va_coupon_data['coupon'] ) ?>]</strong>. 
                        
                    </p>
                    <p>
                        Click <a href="<?php echo esc_attr( $va_coupon_data['register_url'] ) ?>" target="_blank">here</a> to register for validate addresses using the promo code now.
                        If you already connected then please click 
                        <a href="javascript:void(0)" onclick="en_unishippers_small_va_connection_status_refresh(this);">this link</a> 
        ...
        ?>
            <div id="message" class="en-coupon-code-div woocommerce-message en-coupon-success">
                <p>
                    <strong>Congratulations!</strong> You have activated your Promo Code <strong>[<?php echo esc_attr( $va_coupon_data['coupon'] ) ?>]</strong><?php echo esc_attr( $va_coupon_data['va_company_text'] ) ?>. Now you can enjoy validate addresses free for one year.
        ...
        <?php
            ...
            <div id="message" class="en-coupon-code-div woocommerce-message en-coupon-notice">
                <p>
                    <strong>Note!</strong> 
                    Get validate addresses free for one year by using promo code <strong>[<?php echo esc_attr( $va_coupon_data['coupon'] ) ?>]</strong>.
                </p>
                <div class='en-coupon-btn-div'>
                    <button class="en_va_apply_promo_btn button" data-coupon="<?php echo esc_attr( $va_coupon_data['coupon'] ) ?>"><?php esc_html_e( 'Apply Promo Code', 'eniture-unishippers-small-quotes' ); ?></button>
                </div>
            </div>
        <?php
        }
