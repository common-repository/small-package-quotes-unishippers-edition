...
if (!class_exists('Unishippers_EnUnishippersSpqLogs')) {
    class Unishippers_EnUnishippersSpqLogs
    {
        ...
        {
            $obj_classs = new \Unishippers_Small_Curl_Request();
            $carrier_name = get_option('api_endpoint_unishippers_small') == 'unishippers_small_new_api' ? 'Unishippers Small New API' : 'unisheppers';
            $data = array(
                'serverName' => unishippers_small_get_domain(),
                'licenseKey' => get_option('unishepper_small_licence_key'),
                'lastLogs' => '25',
            ...
            $url = get_option('api_endpoint_unishippers_small') == 'unishippers_small_new_api'  ? UNISHIPPERS_NEW_API_DOMAIN_HITTING_URL : UNISHIPPERS_DOMAIN_HITTING_URL;
            $url .= '/request-log/index.php';
            $logs = $obj_classs->unishepper_small_get_curl_response($url, $data);
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $class_name = 'unishippers-small-log-' . $key . rand(1, 100);
                    ...
                    if (isset($request['carrierName']) && strtolower($request['carrierName']) == 'wwe smpkg') {
                    ...
                    if (isset($temp_resp->binPackaging) && !empty($temp_resp->binPackaging)) {
                        ...
                        $bin_packaging = new \Unishippers_EnUnishippersSpqLogsPackaging();
                    ...
                    $action_links = '<td class="log_action_links"><a href = "#en_jtv_showing_res" class="response" onclick=\'en_jtv_res_detail(' . esc_attr( $response) . ')\'>' . esc_attr( $status ) . '</a>';

                    if (!empty($bin_response)) {
                        $box_icon = plugins_url('en-packaging-details/box-icon.png', __FILE__);
                        $packaging_link = ' <span> | </span> <a title="View packaging details" href="#en_jtv_showing_res" class="response" onclick=\'en_packging_details(' . wp_json_encode($bin_packaging_markup) . ')\' ><img src=' . esc_url_raw( $box_icon ) . ' /></a>';
    ...
    new Unishippers_EnUnishippersSpqLogs();
}