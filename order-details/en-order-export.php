 ...
 * Order export.
 */
if (!class_exists('Unishippers_EnOrderExport')) {

    class Unishippers_EnOrderExport
    {
        ...
        public function en_async_cron_schedule($schedules)
        {
            $schedules['en_async_cron_every_5_minute'] = array(
                'interval' => 60 * 5,
                'display' => __('Every 5 minute', 'eniture-unishippers-small-quotes'),
            );
            return $schedules;
        }
            ...
            try {
                ...
                if ($en_orders_ids_flag == 'executed' && $en_async_orders_exporting_process != 'completed') {
                    ...
                    if (!empty($en_orders_id) && is_array($en_orders_id)) {
                        ...
                        update_option('en_orders_ids', wp_json_encode($en_remain_orders_id));
                    ...
                    $post_content = 'Note! Exported Orders : ' . wp_json_encode($en_orders_ids);
        ...
        public function en_orders_ids()
        {
            try {
                ...
                if ($en_orders_ids_flag != 'executed') {
                    ...
                    update_option('en_orders_ids', wp_json_encode($orders));
                    update_option('en_orders_ids_flag', 'executed');

                    $post_content = 'Note! Total Orders Ids: ' . wp_json_encode($orders);
                    $this->en_custom_post_save($post_content);
                    ...
                    if (is_array($data) && !empty($data)) {
                        foreach ($data as $ship_nbr => $shipment) {
                            ...
                            $en_orders[] = [
                                ...
                                'orderMeta' => base64_encode(wp_json_encode($shipment))
                            ];
                        }
                    }
                }
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            ...
            $curl_obj = new Unishippers_Small_Curl_Request();
            $response = $curl_obj->unishepper_small_get_curl_response($url, $post_data);
            $post_content = "Response: " . $response . "<br>Count: " . count($en_orders_ids) . "<br>Orders ID: " . wp_json_encode($en_orders_ids);
            $this->en_custom_post_save($post_content);
            return $response;
        }
    }

    new Unishippers_EnOrderExport();
}