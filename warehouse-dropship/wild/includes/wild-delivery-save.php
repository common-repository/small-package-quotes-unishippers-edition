...
if (!class_exists("Unishippers_EnWooWdAddonsAjaxReqIncludes")) {

    class Unishippers_EnWooWdAddonsAjaxReqIncludes
    {
        ...
        {
            add_action('wp_ajax_nopriv_en_wd_get_address', array($this, 'unishippers_get_address_api_ajax'));
            add_action('wp_ajax_en_wd_get_address', array($this, 'unishippers_get_address_api_ajax'));

            add_action('wp_ajax_nopriv_en_wd_delete_dropship', array($this, 'unishippers_delete_dropship_ajax'));
            add_action('wp_ajax_en_wd_delete_dropship', array($this, 'unishippers_delete_dropship_ajax'));

            add_action('wp_ajax_nopriv_en_wd_save_warehouse', array($this, 'unishippers_save_warehouse_ajax'));
            add_action('wp_ajax_en_wd_save_warehouse', array($this, 'unishippers_save_warehouse_ajax'));

            add_action('wp_ajax_nopriv_en_wd_save_dropship', array($this, 'unishippers_save_dropship_ajax'));
            add_action('wp_ajax_en_wd_save_dropship', array($this, 'unishippers_save_dropship_ajax'));


            add_action('wp_ajax_nopriv_en_wd_edit_dropship', array($this, 'unishippers_edit_dropship_ajax'));
            add_action('wp_ajax_en_wd_edit_dropship', array($this, 'unishippers_edit_dropship_ajax'));

            add_action('wp_ajax_nopriv_en_wd_delete_warehouse', array($this, 'unishippers_delete_warehouse_ajax'));
            add_action('wp_ajax_en_wd_delete_warehouse', array($this, 'unishippers_delete_warehouse_ajax'));

            add_action('wp_ajax_nopriv_en_wd_edit_warehouse', array($this, 'unishippers_edit_warehouse_ajax'));
            add_action('wp_ajax_en_wd_edit_warehouse', array($this, 'unishippers_edit_warehouse_ajax'));
        }
        ...
        function unishippers_save_warehouse_ajax()
        {
            ...
            if (isset($_POST['origin_country']) && $_POST['origin_country'] != '') {
                $origin_contry_code = sanitize_text_field( wp_unslash( $_POST['origin_country']));
                $countrycode = strtolower($origin_contry_code);
                $origin_country = ($countrycode == 'un') ? 'US' : $origin_contry_code;
            }
            ...
            $input_data_arr = array(
                ...
                'suppress_local_delivery' => (isset($_POST['supppress_delivery']) && $_POST['supppress_delivery'] === 'true') ? 1 : 0,
                'origin_markup'=> isset($_POST['origin_markup']) ? sanitize_text_field( wp_unslash($_POST['origin_markup'])) : '',
                'fee_store_pickup' => isset($_POST['fee_instore']) ? sanitize_text_field( wp_unslash($_POST['fee_instore'])) : '',
            );
                ...
                if (isset($validateData["city"])) {
                    ...
                    if ($get_warehouse_id && (empty($get_warehouse) || (!empty($get_warehouse) && reset($get_warehouse)->id == $get_warehouse_id))) {
                        ...
                        if (empty($get_warehouse)) {
                            ...
                            $html = unishippers_warehouse_template(TRUE);
                        }
                    }
                }
        ...
        function unishippers_save_dropship_ajax()
        {
            ...
            if (isset($_POST['dropship_country']) && $_POST['dropship_country'] != '') {
                $dropship_contry_code = sanitize_text_field( wp_unslash( $_POST['dropship_country']));
                $countrycode = strtolower($dropship_contry_code);
                $dropship_contry_code = ($countrycode == 'un') ? 'US' : $dropship_contry_code;
            }
                ...
                if (isset($validateData["city"])) {
                    ...
                    if ($get_dropship_id != '' && (empty($get_dropship) || (!empty($get_dropship) && reset($get_dropship)->id == $get_dropship_id))) {
                        ...
                        if (empty($get_dropship)) {
                            ...
                            $html = unishippers_dropship_template(TRUE);
                        }
                    }
                }
                $lastid = $wpdb->insert_id;
        ...
        function unishippers_delete_dropship_ajax()
        {
            ...
            $wpdb->delete($wpdb->prefix . "warehouse", array('id' => $get_dropship_id, 'location' => 'dropship'));
            $html = unishippers_dropship_template(TRUE);
            echo wp_json_encode($html);
            exit;
        }
...
new Unishippers_EnWooWdAddonsAjaxReqIncludes();
